"""
TikTok SMS Module v13.9

Changelog v13.9:

  Fix #1: Device registration logging. When TikTok returns JSON
    without "message" field, now logs full response snippet (truncated)
    including error_code, code, data fields — instead of just "?".

  Fix #2: aiohttp redirect on registration hosts. Added
    allow_redirects=False to _register_device and _send_xlog requests.
    TikTok registration endpoints sometimes redirect to alternate
    hostnames (e.g. log16-normal-useast5.tiktokv.us) which proxies
    cannot resolve/connect to. We now detect 3xx responses, extract
    the Location header, and try the redirected host directly in the
    host rotation loop — without following the redirect through proxy.

  Fix #3: Prewarm and registration failure logging. Added detailed
    per-host error context to FAIL log lines so root cause (proxy
    block, SSL error, redirect, rate-limit) is immediately visible
    without scrolling back through logs.

  Inherited from v13.8:
    Binary payload signing fix (_classify_body).
"""
...
def extract_error_code(data: dict, context: str = '') -> int:
    # Check nested data.error_code / data.status_code
        ...
        for key in ('error_code', 'status_code'):
            ...
            if raw is not None:
                try:
                    ...
                    pass
    # Check root level error_code / status_code
    for key in ('error_code', 'status_code'):
        raw = data.get(key)
        if raw is not None:
            try:
