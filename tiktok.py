...
# tiktok-signer defaults (TikTok 37.0.4).
# These match the library's built-in defaults;
# passed explicitly so they stay pinned even if
# the library updates its defaults.
SIGNER_LC_ID = 2142840551
SIGNER_SDK_VER = 'v05.01.02-alpha.7-ov-android'
...
# APK versions: (version_name, version_code_str,
#                manifest_version_code)
#
# Only 37.x series — matches tiktok-signer's
# tested version and signing algorithm.
# 39.x removed: signer was built/tested against
# 37.0.4; mixing versions risks X-Argus mismatch
# (error 1105/1109). Re-add only after confirming
# signer compatibility with newer APKs.
CURRENT_VERSIONS: Tuple[Tuple[str, str, str], ...] = (
    ('37.0.4', '370004', '2023700040'),
    ('37.0.3', '370003', '2023700030'),
    ('37.0.2', '370002', '2023700020'),
)
...
# genuine build.
# Value '194326e...' matches tiktok-signer's
# tested APK (see its README device_register
# example). Change only if TikTok rotates their
# Play Store signing key (extremely rare — last
# known change: never).
CURRENT_SIG_HASH = (
    '194326e82c84a639a52e5c023116f12a')

# passport-sdk-version sent as query parameter.
# Version 21 causes 1003/1004 errors on current
# API; 25 is the minimum accepted as of 2026.
...
class AppConfig:
    ...
    def random_version(
            self) -> Tuple[str, str, str]:
        return random.choice(CURRENT_VERSIONS)
...
class Device:
    ...
    def generate(
        ...
        ver = config.random_version()
        ver_name, ver_code, manifest = ver
        return cls(
...
class TikTokSMS:
    ...
    def _generate_auth_headers(
        ...
        auth = TikTokSigner.generate_headers(
            ...
            aid=int(APP_AID),
            lc_id=SIGNER_LC_ID,
            sdk_ver=SIGNER_SDK_VER,
            sdk_ver_code=SIGNER_SDK_VER_CODE,
            version_name=(
                self.device.version_name),
            version_code=(
                self.device.manifest_int),
            cookie=(
                cookie_str or None),
            unix=int(time.time()),
        )
            ...
            async with sem:
                ...
                try:
                    ...
                    async with session.post(url, **kw) as resp:
                        raw = await resp.read()
                        log.info(f'{tag} HTTP {resp.status} len={len(raw)}')
                        log.info(f'{tag} raw={raw[:500]}')
                        data = None
                        try:
                            data = json.loads(raw)
                        except (json.JSONDecodeError, ValueError):
                            try:
                                data = json.loads(TikTokSigner.decrypt(raw))
                            except Exception:
                                pass

                        if (data
                                and isinstance(data, dict)
                                and data.get('device_id', 0)):
                            device.device_id = str(data['device_id'])
                            device.install_id = str(data.get('install_id', 0))
                            device.registered = True
                            await pool.add(device)
                            await proxy_mgr.report_success(proxy_url)
                            log.info(f'{tag} ✓ {device.device_id}')
                        else:
                            await proxy_mgr.report_failure(proxy_url)
                            log.warning(f'{tag} ✗ failed')
