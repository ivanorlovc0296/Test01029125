...
SignerPy.sign() accepts url and params as SEPARATE arguments:
  sign(params=query_string, url=base_url, payload=body, version=8404)

Device file format (one per line, colon-separated):
...
ENDPOINT_TIER_3 = SMSEndpoint(
    ...
    notes='Stable for recovery and change_phone.',
)
...
ENDPOINT_TIER_4 = SMSEndpoint(
    ...
    notes='Legacy. Works in some regions.',
)
...
def build_body_for_endpoint(
    ...
    if endpoint.body_style == 'phone_number':
        ...
        body_params['phone_number'] = full_number
...
def signer_sign(
    ...
    SignerPy.sign() accepts url and params as
    SEPARATE arguments:
      sign(params=query_string, url=base_url,
           payload=body, version=8404)

    params is the URL-encoded query string
    WITHOUT the URL path.
    ...
    Returns:
        Dict with x-gorgon, x-argus, etc.
    ...
    try:
        ...
        if payload is not None:
            sign_kwargs['payload'] = payload

        result = signerpy_sign(**sign_kwargs)
...
def signer_headers(
    ...
    SignerPy.sign() receives url and params as
    SEPARATE arguments. url is the base URL
    (scheme+host+path) WITHOUT query string.
    params_str is the URL-encoded query string.
...
class TikTokSMS:
    ...
    def _generate_auth_headers(
        ...
        url is the full URL (scheme+host+path)
        WITHOUT query string.
        params_str is the URL-encoded query.
        They are passed SEPARATELY to SignerPy.
        """
