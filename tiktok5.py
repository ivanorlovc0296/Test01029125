...
SignerPy.sign() requires full device fingerprint parameters
in the params string (version_name, version_code, device_id,
aid, os_version, device_type, etc.) — it parses them internally
for x-argus/x-gorgon generation.
...
Usage:
    ...
    python tiktok_sms.py -i numbers.csv --event recovery --device-file devices.txt

Device file format (one per line):
    device_id:install_id:device_type:device_brand:openudid:cdid
...
# Minimum params required by SignerPy.sign()
# internally for x-argus generation.
# SignerPy parses the params query string and
# reads these keys from parsed dict.
SIGNERPY_REQUIRED_PARAMS = (
    'version_name',
    'version_code',
    'device_id',
    'aid',
...
def _build_minimal_params_str(
        device_id: str = '7123456789012345678',
        ver: Optional['APKVersion'] = None,
) -> str:
    """Build a minimal params query string that
    satisfies SignerPy.sign() internal parsing.

    SignerPy.sign() parses the params string as
    a URL query and reads version_name,
    version_code, device_id, aid, etc. from it
    for x-argus generation. Without these keys
    it crashes with KeyError.

    This function is used ONLY for dependency
    checks and auto-detection — real requests
    use Device.base_params() which already
...
def _validate_params_for_signer(
        params_str: str) -> bool:
    """Check if params string contains all keys
    required by SignerPy.sign() internally."""
    parsed = parse_qs(params_str)
    missing = []
    for key in SIGNERPY_REQUIRED_PARAMS:
        if key not in parsed:
            missing.append(key)
    if missing:
        log.debug(
            'Params missing for SignerPy: %s',
            ', '.join(missing[:5]))
        return False
    return True
...
def signer_sign(
    ...
    IMPORTANT: SignerPy.sign() parses the params
    string internally and reads device fingerprint
    fields (version_name, version_code, device_id,
    aid, os_version, device_type, etc.) from it.
    The params string MUST contain these fields or
    sign() will crash with KeyError.

    Use Device.base_params() → urlencode() to
    build the params string for real requests.
    Use _build_minimal_params_str() for tests.

    Args:
        params: URL-encoded query string with
            ALL device fingerprint parameters
    ...
    try:
        ...
        raise SignerError(
            'sign() missing param: '
            + str(exc)
            + '. Ensure params string contains'
            ' all device fingerprint fields'
...
def signer_headers(
    ...
    The params_str MUST already contain all
    device fingerprint fields because SignerPy
    parses them internally.

    The url is the full request URL (scheme +
    host + path) WITHOUT query string. SignerPy
    needs it to match the signed URL.

    Args:
        params_str: URL-encoded query string
            (from Device.base_params() urlencode)
        url: Full URL without query string
            e.g. https://api16.../passport/...
    ...
    # Build the full URL with params for signing
    # SignerPy.sign() needs the complete URL
    # to generate matching x-gorgon/x-argus
    sign_url = url + '?' + params_str
...
def detect_signer_version() -> Optional[APKVersion]:
    ...
    for ver in APK_VERSIONS:
        ...
        try:
            ...
            log.debug(
                'Auto-detect %s failed: %s',
                ver.version_name, exc)
            continue
...
# ============================================
#  EXTERNAL DEVICE FILE LOADER
# ============================================

@dataclass
class ExternalDevice:
    """A device loaded from an external file.

    File format (one per line, colon-separated):
      device_id:install_id:device_type:device_brand:openudid:cdid
...
def load_external_devices(
        path: str) -> List[ExternalDevice]:
    """Load devices from a text file.

    Each line must have 6 colon-separated fields:
      device_id:install_id:device_type:device_brand:openudid:cdid

    Lines starting with # are skipped.
    Empty lines are skipped.
    Lines with wrong field count are logged and skipped.

    Args:
        path: Path to the device file.

    Returns:
        List of ExternalDevice objects.
    ...
    with open(path, 'r') as f:
        for line_num, line in enumerate(f, 1):
            ...
            if len(parts) < 6:
                ...
                skipped += 1
                continue
            # Take exactly the first 6 fields
            # (in case cdid contains colons —
            #  rejoin remaining into cdid)
            device_id = parts[0].strip()
            install_id = parts[1].strip()
            device_type = parts[2].strip()
            device_brand = parts[3].strip()
            openudid = parts[4].strip()
            # cdid might contain colons
            # (UUID format), rejoin remaining
            cdid = ':'.join(
    ...
    if skipped:
        log.warning(
            'Device file: skipped %d lines',
            skipped)
    log.info(
        'Device file: loaded %d devices from %s',
        len(devices), path)
    return devices


class ExternalDevicePool:
    """Thread-safe round-robin pool of
    externally loaded devices."""
...
def build_device_from_external(
        ext: ExternalDevice,
        config: AppConfig,
        region: str = 'US',
) -> 'Device':
    """Build a full Device from an ExternalDevice
    entry, filling in APK version, OS, DPI, etc.
    from config.

    The device_id, install_id, device_type,
    device_brand, openudid, and cdid come from
    the external file. Everything else is
    generated/selected from config.

    Args:
        ext: External device entry
        config: App configuration
        region: Country/region code

    Returns:
        A fully populated Device instance with
        registered=True (external devices are
        assumed to be pre-registered).
    """
    ver = config.get_default_apk_version()

    # Try to find matching hardware specs for
    # the device_type from config.devices
    # (brand match + type match)
    matched_hw = None
    ext_brand_lower = ext.device_brand.lower()
    ext_type_lower = ext.device_type.lower()
    for hw in config.devices:
        hw_brand = hw[0].lower()
        hw_type = hw[1].lower()
        if (hw_brand == ext_brand_lower
                and hw_type == ext_type_lower):
            if ver.is_device_compatible(
                    int(hw[2])):
                matched_hw = hw
                break

    # If no exact match, try brand-only match
    if matched_hw is None:
        for hw in config.devices:
            hw_brand = hw[0].lower()
            if hw_brand == ext_brand_lower:
                if ver.is_device_compatible(
    ...
    # If still no match, pick a compatible one
    if matched_hw is None:
        compatible = [
            ...
            if ver.is_device_compatible(
                int(d[2]))]
        if compatible:
            matched_hw = random.choice(compatible)
        else:
            matched_hw = config.devices[0]

    os_api = matched_hw[2]
    os_version = matched_hw[3]
    dpi = matched_hw[4]
    resolution = matched_hw[5]

    return Device(
        device_id=ext.device_id,
        install_id=ext.install_id,
        openudid=ext.openudid,
        cdid=ext.cdid,
        device_type=ext.device_type,
        device_brand=ext.device_brand.capitalize(),
...
class Device:
    ...
    def base_params(self) -> Dict[str, str]:
        """Build full device fingerprint params.

        This dict contains ALL fields required by
        SignerPy.sign() internally — it parses
        the urlencode'd string and reads
        version_name, version_code, device_id,
        aid, os_version, device_type, etc.
        """
        ver = self.apk_version
        now_ms = str(int(time.time() * 1000))
        now_s = str(int(time.time()))
...
class TikTokSMS:
    ...
    def __init__(
            ...
            pool: DevicePool,
            limiter: RateLimiter,
            ext_device_pool: Optional[
                ExternalDevicePool] = None):
        self.config = config
        ...
        self.ext_device_pool = ext_device_pool
    ...
    def _generate_auth_headers(
        ...
        params_str must be built from
        Device.base_params() which contains all
        fields SignerPy needs internally.

        url is the full URL (scheme + host + path)
    ...
    async def _get_or_create_device(
        ...
        # Try external device pool first
        if (self.ext_device_pool
                and not self.ext_device_pool
                .is_empty):
            ext = (
                await self.ext_device_pool
                .get_next())
            if ext:
                self.device = (
                    build_device_from_external(
                        ext, self.config,
                        country))
    ...
    async def _get_lightweight_device(
        ...
        # Generate lightweight (disposable)
        self.device = (
    ...
    async def _send_on_endpoint(
        ...
        for attempt in range(
            ...
            if ec == TT_DEVICE_BAN:
                ...
                # Remove banned external device
                if (self.ext_device_pool
                        and self.device):
                    await (
                        self.ext_device_pool
                        .remove(
                            self.device
                            .device_id))
                if is_lightweight:
...
async def process_csv(
    ...
    # Load external devices if configured
    ext_pool: Optional[
        ExternalDevicePool] = None
    if config.device_file:
        try:
            ext_devices = (
                load_external_devices(
                    config.device_file))
            if ext_devices:
                ext_pool = ExternalDevicePool(
                    ext_devices)
                log.info(
...
def check_dependencies(
    ...
    try:
        ...
        # Build full params for sign() test
        test_params = (
            _build_minimal_params_str(ver=ver))

        # Test sign() with full params
        result = signer_sign(
            params=test_params,
            payload=None,
            gorgon_version=ver.gorgon_version,
