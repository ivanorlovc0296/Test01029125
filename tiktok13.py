"""
TikTok SMS Module v15.0

Changelog v15.0 (based on tech review of v14.0):

  CRITICAL FIX: Timezone consistency
    - Removed hardcoded 'America/New_York' / -14400 timezone.
    - Added REGION_TIMEZONE_MAP (40+ countries) and get_timezone_for_region().
    - base_params(), registration_payload(), xlog_payload(), _build_test_params()
      now use dynamic timezone matching the device region.
    - Root cause of ec=7 on non-US numbers (IT, DE, FR, etc.).

  FIX: ec=7 handling in _send_on_endpoint()
    - ec=7 now triggers device+proxy rotation and retry instead of
      immediate fail with no fallback.
    - Added TT_ENVIRONMENT_REJECT = 7 constant.

  FIX: SignerPy params format
    - sign() now receives params as dict via url_to_params.dicts(),
      not as raw query string. Per SignerPy 0.12.0 PyPI documentation.
    - Removed SIGNER_URL_MODE / _make_sign_params() (no longer needed).
    - get() also receives dict params.

  OPTIMIZATION: Lightweight device path for recovery/change_phone
    - _get_lightweight_device() skips register, xlog, msToken when
      no external pool is available. Generates device params only.
    - Saves 3-5 wasted HTTP requests per phone number.
    - Recovery proceeds directly to send_code with unregistered device.

  FIX: _ensure_ms_token removed deprecated /passport/app/region_setting/
    - Endpoint returns 404 on v37+. Replaced with /passport/user/info/ only
      plus cookie jar extraction fallback.

  Carries forward all v14.0/v13.9 fixes.
"""
...
try:
    from SignerPy import url_to_params as signerpy_url_to_params
    HAS_URL_TO_PARAMS = True
except ImportError:
    HAS_URL_TO_PARAMS = False
...
# ec=7 handling constants
EC7_BACKOFF_MIN = 5.0
EC7_BACKOFF_MAX = 15.0
EC7_MAX_ROTATIONS_PER_ENDPOINT = 2
...
# MS Token refresh endpoints (v15.0: removed deprecated region_setting)
MS_TOKEN_REFRESH_ENDPOINTS = ('/passport/user/info/',)


# ============================================
#  REGION TIMEZONE MAP (v15.0)
# ============================================

REGION_TIMEZONE_MAP: Dict[str, Tuple[str, int]] = {
    'US': ('America/New_York', -18000),
    'GB': ('Europe/London', 0),
    'DE': ('Europe/Berlin', 3600),
    'FR': ('Europe/Paris', 3600),
    'IT': ('Europe/Rome', 3600),
    'ES': ('Europe/Madrid', 3600),
    'PT': ('Europe/Lisbon', 0),
    'NL': ('Europe/Amsterdam', 3600),
    'BE': ('Europe/Brussels', 3600),
    'AT': ('Europe/Vienna', 3600),
    'CH': ('Europe/Zurich', 3600),
    'PL': ('Europe/Warsaw', 3600),
    'CZ': ('Europe/Prague', 3600),
    'SE': ('Europe/Stockholm', 3600),
    'NO': ('Europe/Oslo', 3600),
    'DK': ('Europe/Copenhagen', 3600),
    'FI': ('Europe/Helsinki', 7200),
    'GR': ('Europe/Athens', 7200),
    'RO': ('Europe/Bucharest', 7200),
    'RU': ('Europe/Moscow', 10800),
    'UA': ('Europe/Kiev', 7200),
    'TR': ('Europe/Istanbul', 10800),
    'IL': ('Asia/Jerusalem', 7200),
    ...
    'CN': ('Asia/Shanghai', 28800),
    'TW': ('Asia/Taipei', 28800),
    'HK': ('Asia/Hong_Kong', 28800),
    'JP': ('Asia/Tokyo', 32400),
    'KR': ('Asia/Seoul', 32400),
    'AU': ('Australia/Sydney', 36000),
    'NZ': ('Pacific/Auckland', 43200),
    'BR': ('America/Sao_Paulo', -10800),
    'AR': ('America/Argentina/Buenos_Aires', -10800),
    'CL': ('America/Santiago', -10800),
    'CO': ('America/Bogota', -18000),
    'PE': ('America/Lima', -18000),
    'MX': ('America/Mexico_City', -21600),
    'CA': ('America/Toronto', -18000),
    'EG': ('Africa/Cairo', 7200),
    'NG': ('Africa/Lagos', 3600),
    'ZA': ('Africa/Johannesburg', 7200),
    'KE': ('Africa/Nairobi', 10800),
}

_DEFAULT_TIMEZONE = ('America/New_York', -18000)


def get_timezone_for_region(region: str) -> Tuple[str, int]:
    """Returns (timezone_name, offset_seconds). Defaults to America/New_York."""
    return REGION_TIMEZONE_MAP.get(region.upper(), _DEFAULT_TIMEZONE)
...
EVENT_ENDPOINT_MAP: Dict[str, EventEndpointConfig] = {
    ...
    'recovery': EventEndpointConfig(5, (5,), (3,)),
...
# ============================================
#  SIGNERPY WRAPPERS (v15.0: dict params)
# ============================================
...
def _query_string_to_dict(qs: str) -> dict:
    """Convert 'key1=val1&key2=val2' to {'key1': 'val1', 'key2': 'val2'}.
    Used as fallback when SignerPy.url_to_params is unavailable."""
    result = {}
    if not qs:
        return result
...
def _url_to_params_dict(full_url: str) -> dict:
    """Convert full URL to params dict for SignerPy.
    Uses SignerPy.url_to_params.dicts() if available, else manual parse."""
    if HAS_URL_TO_PARAMS:
        try:
            result = signerpy_url_to_params.dicts(full_url)
            if isinstance(result, dict) and result:
                return result
        except Exception as exc:
            log.debug('url_to_params.dicts() failed: %s', exc)
    # Fallback: manual parse
    parsed = urlparse(full_url)
    return _query_string_to_dict(parsed.query)


def signer_sign(params, payload: str = '',
                version: int = SIGNER_GORGON_VERSION) -> Dict[str, str]:
    """Sign request. params must be a dict (v15.0 fix)."""
    try:
        if isinstance(params, str):
            params = _query_string_to_dict(params)
...
def signer_get(params) -> Dict[str, str]:
    """GET signer. params must be a dict (v15.0 fix)."""
    if isinstance(params, str):
        params = _query_string_to_dict(params)
    if HAS_SIGNERPY_GET:
        try:
...
def _build_signing_headers(params_dict: dict,
                           body_info: _BodyInfo,
                           user_agent: str,
                           frozen_ts_s: Optional[str] = None,
                           frozen_ts_ms: Optional[str] = None,
                           ) -> Dict[str, str]:
    """Build signed headers. params_dict is a dict (v15.0)."""
    if body_info.is_post and body_info.sign_payload:
        auth = signer_sign(params=params_dict, payload=body_info.sign_payload)
    else:
        auth = signer_get(params=params_dict)
...
def _build_test_params(device_id='7123456789012345678', ver=None):
    ...
    tz_name, tz_offset = get_timezone_for_region('US')
    return {
...
class AppConfig:
    ...
    device_file: str = ''
    skip_register_for_lightweight: bool = True  # v15.0
...
class Device:
    ...
    def _get_timezone(self) -> Tuple[str, int]:
        """Get timezone matching this device's region."""
        return get_timezone_for_region(self.region)
    ...
    def base_params(self, frozen_ts_ms=None, frozen_ts_s=None):
        ...
        tz_name, tz_offset = self._get_timezone()
        p = {
            'passport-sdk-version': ver.passport_sdk_version,
    ...
    def base_params_dict(self, frozen_ts_ms=None, frozen_ts_s=None) -> dict:
        """Return base params as dict (for SignerPy). Same as base_params."""
        return self.base_params(frozen_ts_ms, frozen_ts_s)

    def base_query_string(self, frozen_ts_ms=None, frozen_ts_s=None):
        return urlencode(self.base_params(frozen_ts_ms, frozen_ts_s))

    def captcha_params_dict(self, extra, frozen_ts_ms=None, frozen_ts_s=None) -> dict:
        """Return captcha params as dict for signing."""
    ...
    def registration_payload(self):
        ...
        return {
            ...
            'header': {
                ...
                'language': 'en', 'timezone': tz_offset // 3600,
                'access': 'wifi',
...
# ============================================
#  TIKTOK SMS ENGINE (v15.0: dict params, ec=7, lightweight skip)
# ============================================
...
class TikTokSMS:
    ...
    async def _request(self, method, path, body=None,
        ...
        """Core HTTP request with signing. v15.0: params as dict to signer."""
        ...
        for attempt in range(self.config.max_retries):
            ...
            # v15.0: build params as dict for signing
            params_dict = self.device.base_params_dict(frozen_ts_ms, frozen_ts_s)
            params_qs = urlencode(params_dict)
            full_url = base_url + '?' + params_qs

            # v15.0: pass dict to signer, not query string
    ...
    async def _ensure_ms_token(self, proxy_url=None, phone=''):
        """Try to obtain msToken. v15.0: removed deprecated region_setting."""
        if self.device.ms_token_valid:
            return True, ''
        self.device.invalidate_ms_token()
        errors = []
        for ep in MS_TOKEN_REFRESH_ENDPOINTS:
            try:
    ...
    async def _solve_captcha_flow(self, subtype, detail, region, verify_event,
        ...
        # v15.0: pass dict to signer
        captcha_params_dict = self.device.captcha_params_dict(
            captcha_extra, frozen_ts_ms, frozen_ts_s)
        captcha_qs = urlencode(captcha_params_dict)

        get_info = _classify_body('GET', None)
        headers = _build_signing_headers(
            captcha_params_dict, get_info, self.device.user_agent, frozen_ts_s, frozen_ts_ms)

        session = await self._get_session()
        try:
            async with session.get(
                URL_CAPTCHA_GET + '?' + captcha_qs, headers=headers,
        ...
        verify_extra = {'aid': APP_AID_STR, 'host': self.api_host}
        verify_params_dict = self.device.captcha_params_dict(
            verify_extra, verify_ts_ms, verify_ts_s)
        verify_qs = urlencode(verify_params_dict)
        ...
        verify_headers = _build_signing_headers(
            verify_params_dict, post_info, self.device.user_agent, verify_ts_s, verify_ts_ms)
        verify_headers['Content-Type'] = 'application/json'

        try:
            async with session.post(
                URL_CAPTCHA_VERIFY + '?' + verify_qs, headers=verify_headers,
    ...
    async def _get_lightweight_device(self, country, proxy_url=None, phone=''):
        """v15.0: for recovery/change_phone, skip register+xlog+msToken
        when no external pool. Just generate device params and go."""
        if await self._checkout_external(country, proxy_url, phone):
            return True
        # Check pool first
        pooled = await self.pool.get(country)
        if pooled:
            ...
            self.device.session_uses = 0
            self._on_device_switch(self.config.random_host())
            # Pooled device is registered; try msToken
            await self._ensure_ms_token(proxy_url, phone)
            return True
        ...
        self.device.session_uses = 0
        self._on_device_switch(self.config.random_host())
        if self.config.skip_register_for_lightweight:
            # v15.0: skip register, xlog, msToken entirely
            # Device has generated IDs — proceed directly to send_code
            log.info('[%s] Lightweight: skip register, direct send',
                     mask_phone(phone))
            return True
        # Legacy path: attempt register (non-critical)
        try:
    ...
    async def _send_on_endpoint(self, phone, endpoint, sms_type, country,
        ...
        """v15.0: added ec=7 handling with device+proxy rotation."""
        masked = mask_phone(phone)
        ...
        for attempt in range(self.config.max_retries):
            ...
            if self.device.ms_token_valid or not is_lw:
                await self._ensure_ms_token(proxy_url, phone)
            ...
            # v15.0: ec=7 environment rejection — rotate device + proxy
            if ec == TT_ENVIRONMENT_REJECT:
                ec7_rotations += 1
                log.warning('[%s] ec=7 tier-%d (rotation %d/%d): environment rejection',
                            masked, endpoint.tier, ec7_rotations,
                            EC7_MAX_ROTATIONS_PER_ENDPOINT)
                if ec7_rotations > EC7_MAX_ROTATIONS_PER_ENDPOINT:
                    # Exhausted rotations on this endpoint — signal fallback
                    return {'status': 'fail', 'error_code': ec,
                            'timestamp': int(time.time()),
                            'endpoint_tier': endpoint.tier}, True
                # Rotate device
                if is_lw:
                    await self._rotate_lightweight(country, proxy_url, phone)
                else:
                    await self._rotate_device(country, proxy_url, phone)
                # Rotate proxy
                new_proxy = await self.proxy.get(country)
                if new_proxy:
                    proxy_url = new_proxy
                # Rotate API host
                self.api_host = self.config.random_host()
                await asyncio.sleep(random.uniform(EC7_BACKOFF_MIN, EC7_BACKOFF_MAX))
                continue
...
async def process_api(config, proxies=None, event='register', threads=5,
    ...
    try:
        while not is_shutdown():
            if max_numbers > 0 and total_processed >= max_numbers:
                log.info('Reached max_numbers=%d, stopping', max_numbers)
                break
            this_claim = claim_count
            if max_numbers > 0:
                remaining = max_numbers - total_processed
                this_claim = min(claim_count, remaining)
                if this_claim <= 0:
                    break
            try: cd = await number_api.claim(api_session, this_claim, lease_seconds)
            ...
            if not cd or not cd.get('items'):
                ...
                await asyncio.sleep(min(2 ** empty_streak, 30)); continue
            empty_streak = 0
            total_processed += len(cd.get('items', []))
...
# ============================================
#  DEPENDENCY CHECK (v15.0: dict params)
# ============================================
...
def check_dependencies(apk_ver=None):
    try:
        ...
        if HAS_URL_TO_PARAMS: modules.append('url_to_params')
        ...
        # v15.0: test with dict params
        test_params = _build_test_params(ver=ver)
        assert isinstance(test_params, dict), 'test_params must be dict'

        log.info('Testing sign(params=<dict>, payload="", version=%d)...', SIGNER_GORGON_VERSION)
        ...
        # v15.0: test timezone mapping
        tz_it = get_timezone_for_region('IT')
        assert tz_it == ('Europe/Rome', 3600), 'IT timezone wrong: %s' % str(tz_it)
        tz_us = get_timezone_for_region('US')
        assert tz_us == ('America/New_York', -18000), 'US timezone wrong: %s' % str(tz_us)
        tz_xx = get_timezone_for_region('XX')
        assert tz_xx == _DEFAULT_TIMEZONE, 'Unknown region should default'
        log.info('Timezone mapping OK (IT=%s/%d, US=%s/%d)',
                 tz_it[0], tz_it[1], tz_us[0], tz_us[1])

        # v15.0: test that base_params uses region timezone
        test_device = Device()
        test_device.region = 'IT'
        test_device.device_id = '123'
        test_device.install_id = '456'
        test_device.version_name = ver.version_name
        bp = test_device.base_params()
        assert bp['timezone_name'] == 'Europe/Rome', 'IT device should have Rome timezone'
        assert bp['timezone_offset'] == '3600', 'IT device should have +3600 offset'
        log.info('Device base_params timezone consistency OK')
        ...
        log.info('SignerPy OK (ver=%s, gorgon=%d, params_format=dict)',
                 ver.version_name, SIGNER_GORGON_VERSION)
        return True
...
def main():
    _setup_signals()
    parser = argparse.ArgumentParser(description='TikTok SMS v15.0')
    ...
    parser.add_argument('--max-numbers', type=int, default=0,
                        help='Max total numbers to process in API mode (0=unlimited)')
    ...
    parser.add_argument('--no-skip-register', action='store_true',
                        help='Force device registration even for lightweight events')
    ...
    if args.no_skip_register: overrides['skip_register_for_lightweight'] = False
    ...
    if event in LIGHTWEIGHT_EVENTS and config.skip_register_for_lightweight:
        log.info('Lightweight mode: skip register/xlog/msToken')
    ...
    try:
        if args.prewarm:
            ...
            asyncio.run(process_api(config, proxies, event, args.threads, cc, ls,
                                    max_numbers=args.max_numbers,
                                    success_file=args.success, fail_file=args.fail))
        else:
