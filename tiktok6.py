"""
TikTok SMS Module v13.8

Changelog v13.8:

  Fix: Binary payload signing. _request() detects bytes body and:
    1. Signs with payload='' (binary payloads are not included in
       gorgon signature computation — same as TikTok official client)
    2. Computes x-ss-stub as MD5 of raw bytes (not decoded string)
    3. Sends original bytes to server unmodified

  This fixes device registration and XLog where TTEncrypt output
  (binary) was decoded to UTF-8 with replacements, creating a
  mismatch between signed payload and actual body sent to server.
"""
...
def _compute_ss_stub_str(body: str) -> str:
    """x-ss-stub from string body (form-encoded POST)."""
    if not body:
        return ''
...
def _compute_ss_stub_bytes(body: bytes) -> str:
    """x-ss-stub from raw bytes body (encrypted payload)."""
    if not body:
        return ''
...
# v13.8: body_info encapsulates what _request knows about the body
@dataclass(frozen=True)
class _BodyInfo:
    """Describes the body for signing and sending.

    sign_payload: string to pass to sign(payload=...).
      - For text POST (form-encoded): the actual body string.
      - For binary POST (encrypted): empty string (binary payloads
        are not included in gorgon signature).
      - For GET: empty string.

    ss_stub: precomputed x-ss-stub value, or empty.
      - For text POST: MD5 of body string.
      - For binary POST: MD5 of raw bytes.
      - For GET: empty.
...
def _classify_body(method: str, body: Any) -> _BodyInfo:
    ...
    This is the core fix for v13.8: binary (bytes) payloads are NOT
    decoded to UTF-8 for signing. Instead:
    - sign_payload is empty (binary payloads excluded from gorgon sig)
    - ss_stub is MD5 of raw bytes
    - raw_data is the original bytes, sent unmodified
    """
    is_post = method == 'POST'
    ...
    if isinstance(body, bytes):
        # BINARY PATH: encrypted payload (registration, xlog)
        # Do NOT decode — would corrupt data and break signature match.
        return _BodyInfo(
            sign_payload='',  # binary excluded from gorgon
            ss_stub=_compute_ss_stub_bytes(body),
            raw_data=body,
            is_post=is_post)

    if isinstance(body, str):
        # TEXT PATH: form-encoded SMS body, JSON captcha body
        return _BodyInfo(
            sign_payload=body,
...
def _build_signing_headers(params_str: str,
    ...
    """Build HTTP headers with signing.

    Uses body_info to determine whether to call sign() or get(),
    and what x-ss-stub value to use.
    """
    ...
    # Always override x-ss-stub with our computed value
...
class TikTokSMS:
    ...
    # v13.8: _request uses _classify_body to handle binary vs text correctly
    async def _request(self, method, path, body=None,
        ...
        # v13.8: classify body ONCE — determines signing strategy
        ...
        for attempt in range(self.config.max_retries):
            ...
            try:
                ...
                async with fn(full_url, **kw) as resp:
    ...
    async def _register_device(self, proxy_url=None, phone=''):
        ...
        for reg_host in DEVICE_REGISTER_HOSTS:
            check_shutdown()
            ...
            try:
                ...
                log.warning('[%s] Register %s: %s', masked, reg_host, exc)
                continue
            ...
            if did and did != 0:
                ...
                log.info('[%s] Registered: %s via %s', masked, self.device.device_id, reg_host)
                return True
            log.error('[%s] Register %s: %s', masked, reg_host,
    ...
    async def _send_xlog(self, proxy_url=None, phone=''):
        masked = mask_phone(phone)
        for host in DEVICE_REGISTER_HOSTS:
            ...
            try:
                ...
                log.warning('[%s] XLog %s: %s', masked, host, exc)
        return False
    ...
    async def _solve_captcha_flow(self, subtype, detail, region, verify_event,
        ...
        headers = _build_signing_headers(
            captcha_params, get_info, self.device.user_agent, frozen_ts_s, frozen_ts_ms)
        ...
        verify_headers = _build_signing_headers(
            verify_params, post_info, self.device.user_agent, verify_ts_s, verify_ts_ms)
    ...
    async def _send_on_endpoint(self, phone, endpoint, sms_type, country,
        ...
        for attempt in range(self.config.max_retries):
            ...
            if ec == -1 and 'success' in msg:
                return {'status': 'success', 'error_code': 0,
                        'timestamp': int(time.time()), 'endpoint_tier': endpoint.tier}, False
            log.error('[%s] ec=%d tier-%d', masked, ec, endpoint.tier)
            ...
            async with sem:
                ...
                async with TikTokSMS(config, connector, proxy_mgr, captcha, hlr, pool, limiter) as client:
                    ...
                    try:
                        ...
                        else: log.warning('%s FAIL', tag)
                    except Exception as exc: log.error('%s %s', tag, exc)
...
def check_dependencies(apk_ver=None):
    try:
        ...
        # Test POST with text body (SMS send)
        ...
        # Test binary body classification (registration)
        ...
        # Test text body classification (SMS send)
        ...
        try:
            ...
            # Verify encrypted output is bytes and would be classified as binary
        ...
        log.info('SignerPy OK (ver=%s, gorgon=%d)', ver.version_name, SIGNER_GORGON_VERSION)
        return True
...
def main():
    _setup_signals()
    parser = argparse.ArgumentParser(description='TikTok SMS v13.8')
