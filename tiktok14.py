"""
TikTok SMS Module v15.1

Changelog v15.1:

  CRITICAL FIX: sign(params=) must receive FULL URL string.
    - Confirmed by SignerPy author (@asyncL7N).
    - Format: "https://host/path/?key1=val1&key2=val2"
    - Removed SIGNER_URL_MODE, _make_sign_params.
    - _build_signing_headers() now takes full_url string.

  FIX: Endpoint /passport/mobile/send_code/v1/ (with /v1/ suffix).
    - Confirmed by author: tier-3 path must include /v1/.

  FIX: Dynamic timezone per region (REGION_TIMEZONE_MAP).
    - base_params, registration_payload, xlog_payload all use device region.

  FIX: ec=7 (environment rejection) now handled with device+proxy rotation.

  FIX: Lightweight recovery skips register/xlog/msToken by default.

  FIX: Removed deprecated /passport/app/region_setting/ from msToken refresh.

  Carries forward all v13.8/v13.9/v14.0 fixes.
"""
...
# v15.1: only /passport/user/info/ for msToken (region_setting removed/404)
...
def get_timezone_for_region(region: str) -> Tuple[str, int]:
    return REGION_TIMEZONE_MAP.get(
        region.upper() if region else 'US', _DEFAULT_TIMEZONE)
...
ENDPOINT_TIER_3 = SMSEndpoint(
    tier=3, path='/passport/mobile/send_code/v1/',
...
def _build_signing_headers(full_url: str,
                           body_info: _BodyInfo,
                           user_agent: str,
                           frozen_ts_s: Optional[str] = None,
                           frozen_ts_ms: Optional[str] = None,
                           ) -> Dict[str, str]:
    """Build signed headers. full_url is complete URL string for sign(params=)."""
    if body_info.is_post and body_info.sign_payload:
        auth = signer_sign(params=full_url, payload=body_info.sign_payload)
    else:
        auth = signer_get(params=full_url)
...
def _build_test_url(device_id='7123456789012345678', ver=None,
                    host='api16-normal-c-useast1a.tiktokv.com'):
    """Build full URL string for sign() testing."""
    if ver is None:
    ...
    return 'https://' + host + '/passport/mobile/send_code/v1/?' + qs
...
class AppConfig:
    ...
    device_file: str = ''
    skip_register_for_lightweight: bool = True
...
class Device:
    ...
    def build_full_url(self, base_url, frozen_ts_ms=None, frozen_ts_s=None):
        """Build full URL for signing: https://host/path/?key=val&..."""
        qs = self.base_query_string(frozen_ts_ms, frozen_ts_s)
        return base_url + '?' + qs
    ...
    def captcha_full_url(self, captcha_base_url, extra, frozen_ts_ms=None, frozen_ts_s=None):
        """Build full captcha URL for signing."""
        qs = self.captcha_query_string(extra, frozen_ts_ms, frozen_ts_s)
        return captcha_base_url + '?' + qs
    ...
    def registration_payload(self):
        ...
        return {
            ...
            'header': {
                ...
                'language': 'en', 'timezone': tz_offset // 3600, 'access': 'wifi',
...
class TikTokSMS:
    ...
    async def _request(self, method, path, body=None,
        ...
        for attempt in range(self.config.max_retries):
            ...
            # v15.1: build full URL string for signing
            full_url = self.device.build_full_url(base_url, frozen_ts_ms, frozen_ts_s)

            # v15.1: pass full URL to signer
    ...
    async def _solve_captcha_flow(self, subtype, detail, region, verify_event,
        ...
        captcha_full_url = self.device.captcha_full_url(
            URL_CAPTCHA_GET, captcha_extra, frozen_ts_ms, frozen_ts_s)

        get_info = _classify_body('GET', None)
        headers = _build_signing_headers(
            captcha_full_url, get_info, self.device.user_agent, frozen_ts_s, frozen_ts_ms)
        ...
        verify_full_url = self.device.captcha_full_url(
            URL_CAPTCHA_VERIFY, verify_extra, verify_ts_ms, verify_ts_s)
        ...
        verify_headers = _build_signing_headers(
            verify_full_url, post_info, self.device.user_agent, verify_ts_s, verify_ts_ms)
    ...
    async def _get_lightweight_device(self, country, proxy_url=None, phone=''):
        ...
        self.device.session_uses = 0
        self._on_device_switch(self.config.random_host())
        if getattr(self.config, 'skip_register_for_lightweight', True):
        ...
        for attempt in range(self.config.max_retries):
            ...
            if ec == TT_ENVIRONMENT_REJECT:
                ...
                log.warning('[%s] ec=7 tier-%d (rotation %d/%d): environment rejection',
                            masked, endpoint.tier, ec7_rotations, EC7_MAX_ROTATIONS)
                if ec7_rotations > EC7_MAX_ROTATIONS:
    ...
    try:
        while not is_shutdown():
            if max_numbers > 0 and total_processed >= max_numbers:
                ...
                break
            # Adjust claim size if near limit
...
def check_dependencies(apk_ver=None):
    try:
        ...
        test_url = _build_test_url(ver=ver)
        log.info('Test URL: %s...', test_url[:120])

        log.info('Testing sign(params=<full_url>, payload="", version=%d)...', SIGNER_GORGON_VERSION)
        result = signer_sign(params=test_url, payload='')
        ...
        body_result = signer_sign(params=test_url, payload=test_body)
        ...
        # v15.1: verify different URLs produce different argus
        import time as _t; _t.sleep(0.1)
        test_url2 = _build_test_url(ver=ver)
        result2 = signer_sign(params=test_url2, payload='')
        if result2.get('x-argus') == result.get('x-argus'):
            log.warning('x-argus STATIC across calls — possible signer issue')
        else:
            log.info('x-argus varies between calls: OK')
        ...
        log.info('Binary body classification OK')
        ...
        log.info('Text body classification OK')

        if HAS_SIGNERPY_GET:
            try:
                gr = signer_get(params=test_url)
        ...
        assert tz_it == ('Europe/Rome', 3600), 'IT timezone wrong'
        tz_us = get_timezone_for_region('US')
        assert tz_us == ('America/New_York', -18000), 'US timezone wrong'
        ...
        log.info('Response summarizer OK')

        log.info('SignerPy OK (ver=%s, gorgon=%d, params=full_url)',
...
def main():
    _setup_signals()
    parser = argparse.ArgumentParser(description='TikTok SMS v15.1')
    ...
    parser.add_argument('--no-skip-register', action='store_true',
                        help='Force device registration for lightweight events')
