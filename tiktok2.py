...
def validate_gorgon_prefix(
    ...
    level(
        ...
        ' expected %s, got %s.'
        ' Signer may not match APK %s.'
        ' Try: pip install --upgrade'
        ' tiktok-signer',
        tag, ver.gorgon_prefix,
...
class AppConfig:
    ...
    def validate_recovery(
        ...
        If need_number_api is True, also
        checks number_api_base and
        number_api_key (for --api mode).
        """
        missing = []
        if not self.hlr_api_key:
            missing.append('hlr_api_key')
                ...
                if not self.captcha.has_key:
                    ...
                    if (captcha_rotations_without_key
                        ...
                        raise CaptchaError(
                            'No captcha key,'
                            ' rotated '
                            + str(captcha_rotations_without_key)
                            + 'x')
                    log.warning(
                        '[%s] Captcha required'
                        ' but no key,'
...
def main():
    ...
    sms_type = EVENT_SMS_TYPE[event]
    lw_label = (
        'lightweight' if is_lw else 'full')
    log.info(
        'Event: %s (sms_type=%d, %s)',
        event, sms_type, lw_label)
    if is_lw:
        log.info(
            'Lightweight mode: no'
